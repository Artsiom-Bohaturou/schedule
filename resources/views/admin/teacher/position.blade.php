...
@section('content')
    ...
    @php
        $heads = ['#', trans('admin.abbreviated'), trans('admin.teacher_position_full'), ['label' => trans('admin.actions'), 'no-export' => true, 'width' => 5]];
    ...
    {{-- EDIT FORM --}}
    <form method="POST" id="updateForm" action="{{ route('teacher.position.update', 1) }}"> {{-- ID changes on submit --}}
        <x-adminlte-modal id="modalEdit" title="{{ trans('admin.teacher_position_modal_edit_title') }}" theme="warning"
            ...
            <label>{{ trans('admin.teacher_position_full') }}</label>
    ...
    {{-- DELETE FORM --}}
    <form method="POST" id="destroyForm" action="{{ route('teacher.position.destroy', 1) }}"> {{-- ID changes on submit --}}
        <x-adminlte-modal id="modalDelete" title="{{ trans('admin.teacher_position_modal_delete_title') }}" theme="danger"
            icon="fas fa-trash" size='lg'>
            @csrf
            @method('DELETE')
            {{ trans('admin.teacher_position_modal_delete_message') }}
            <x-slot name="footerSlot">
                <div class="mr-auto"></div>
    ...
    {{-- CREATE FORM --}}
    <form method="POST" id="createForm" action="{{ route('teacher.position.store') }}">
        <x-adminlte-modal id="modalCreate" title="{{ trans('admin.teacher_position_modal_create_title') }}" theme="success"
