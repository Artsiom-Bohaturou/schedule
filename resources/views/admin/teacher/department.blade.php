...
@section('content')
    ...
    @php
        $heads = ['#', trans('admin.abbreviated'), trans('admin.teacher_department_full'), ['label' => trans('admin.actions'), 'no-export' => true, 'width' => 5]];
    ...
    {{-- EDIT FORM --}}
    <form method="POST" id="updateForm" action="{{ route('teacher.department.update', 1) }}"> {{-- ID changes on submit --}}
        <x-adminlte-modal id="modalEdit" title="{{ trans('admin.teacher_department_modal_edit_title') }}" theme="warning"
            ...
            <label>{{ trans('admin.teacher_department_full') }}</label>
    ...
    {{-- DELETE FORM --}}
    <form method="POST" id="destroyForm" action="{{ route('teacher.department.destroy', 1) }}"> {{-- ID changes on submit --}}
        <x-adminlte-modal id="modalDelete" title="{{ trans('admin.teacher_department_modal_delete_title') }}"
            theme="danger" icon="fas fa-trash" size='lg'>
            @csrf
            @method('DELETE')
            {{ trans('admin.teacher_department_modal_delete_message') }}
            <x-slot name="footerSlot">
                <div class="mr-auto"></div>
    ...
    {{-- CREATE FORM --}}
    <form method="POST" id="createForm" action="{{ route('teacher.department.store') }}">
        <x-adminlte-modal id="modalCreate" title="{{ trans('admin.teacher_department_modal_create_title') }}"
            theme="success" icon="fas fa-plus" size='lg'>
            @csrf
            <label>{{ trans('admin.abbreviated') }}</label>
            <x-adminlte-input name="abbreviated_name" id="abbreviatedInput" />
            <label>{{ trans('admin.teacher_department_full') }}</label>
